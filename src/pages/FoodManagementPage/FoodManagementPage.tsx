...
import { DeletePage } from "../DeletePage/DeletePage.tsx"; // Đảm bảo rằng DeletePage.tsx tồn tại
...
export const FoodManagementPage = () => {
  ...
  const fetchFoodList = () => {
    fetch("https://quanlyquananapi-production.up.railway.app/api/danhsachmonan")
      ...
      .then((data) => {
        if (data.result === 1) {
          const foodList: FoodItem[] = data.monan.map((mon: FoodItem) => ({
            mon_id: mon.mon_id,
            pl_id: mon.pl_id,
          ...
          showErrorToast("Không thể lấy dữ liệu món ăn");
        }
      })
    ...
    <>
      ...
      <Box className="container mx-auto mb-8">
                  ...
                  <CardContent>
                    ...
                    >
                      <Button
                        ...
                        sx={{ minWidth: "40px" }}
                        onClick={() => {
                          console.log("Món ăn được chọn để chỉnh sửa:", item);
                      ...
                      <Button
                        variant="contained"
                        color={item.mon_trangthai === 1 ? "success" : "warning"}
                        sx={{ minWidth: "40px" }}
                        onClick={() => {
                          setSelectedMon_id(item.mon_id);
                          setOpenOffTheDish(true);
                        }}
                      >
                        {item.mon_trangthai === 1 ? (
                          <VisibilityIcon />
                        ) : (
                          <VisibilityOffIcon />
                        )}
        ...
        <CreatePage
          ...
          onAdd={(newFood) => setFoodItem((prev) => [...prev, newFood])}
        />
        <EditPage
          openEdit={openEdit}
        ...
        <DeletePage
          ...
          mon_id={selectedMon_id}
          onDeleteSuccess={() => {
            fetchFoodList();
          }}
        />
        <OffTheDishPage
          openOffTheDish={openOffTheDish}
          setOpenOffTheDish={setOpenOffTheDish}
          mon_id={selectedMon_id}
          mon_trangthai={
            FoodItem.find((item) => item.mon_id === selectedMon_id)
              ?.mon_trangthai || 0
          }
          onOffSuccess={() => {
            fetchFoodList();
